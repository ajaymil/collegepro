...
def adminlogin():
    ...
    if request.method == 'POST':
        if request.form['uname'] == 'admin' or request.form['password'] == 'admin':
            conn = mysql.connector.connect(user='root', password='', host='localhost', database='1chatbotdb')
            cursor = conn.cursor()
            cur = conn.cursor()
...
def answer():
    if request.method == 'POST':
        ...
        conn3 = mysql.connector.connect(user='root', password='', host='localhost', database='1chatbotdb')
        cur3 = conn3.cursor()
